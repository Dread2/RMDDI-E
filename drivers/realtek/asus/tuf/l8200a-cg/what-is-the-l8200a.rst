Realtek® L8200A Gigabit Ethernet Controller (GbE)
=====================

Overview
--------
The **L8200A** is a ASUS Exclusive Gigabit Ethernet Controller developed by **Realtek Semiconductor Co.**. It is commonly used on ASUS TUF series motherboards.

.. container:: figures-side-by-side

   .. figure:: ../images/asus/l8200a/asus-tuf-gaming-x570-plus-wifi.jpeg
      :alt: ASUS TUF X570 PLUS WiFi motherboard
      :align: left
      :width: 48%

      Figure 1 – ASUS TUF X570 PLUS WiFi motherboard that uses the L8200A PHY.

   .. figure:: ../images/asus/l8200a/l8200a.png
      :alt: L8200A QFN-32 package on ASUS TUF X570 PLUS WiFi motherboard
      :align: right
      :width: 48%

      Figure 2 – L8200A PHY in QFN-32 package on ASUS TUF X570 PLUS WiFi motherboard.

Package and Physical Characteristics
-----------------------------------
- **Package Type:** QFN-32 (Quad Flat No-Lead, 32 pins)
- **Mounting:** Surface-mount directly on the motherboard
- **Thermal:** Central exposed pad for heat dissipation
- **Size:** Compact, designed for space-constrained motherboard layouts

